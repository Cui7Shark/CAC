...
% load('cm_w.mat');
p=roundn(p,-100);
CN_di=roundn(CN_di,-100);
CW_di=roundn(CW_di,-100);
...
%%
%%cm 在膜上的P点的索引列向量
cm_nei_suoyin = zeros(length(CN_di),1);
for i = 1:length(p)
...
cm_wai_suoyin = zeros(length(CW_di),1);
for i = 1:length(p)
...
%%%CM
cell_index =zeros(length(cm_nei_suoyin),2);
for i=1:size(cm_nei_suoyin,1)
    for j=1:size(cm_wai_suoyin,1)
        ...
        if roundn(D,-4) == Mem_d
            cell_index(i,1)=cm_nei_suoyin(i);
            cell_index(i,2)=cm_wai_suoyin(j);%Mem n*2的数组 第一列为内膜点索引，第二列为外膜点索引 //(i,j)是一对在膜上的节点
            break
        end
    end
end
